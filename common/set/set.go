...
// Set uses the data map to contain the common.Hashable objects
...
// New returns a new set. Size must be > 0
...
// Add adds an hashable object to the set, if the item is already
...
func (s *Set) Add(obj Hashable) (result bool, err error) {
	if len(s.data)+1 > s.size && s.size != 0 {
		return false, fmt.Errorf("map size exceeded [%d]", s.size)
	}
...
// Contains returns true if a common.Hashable object is contained
...
// Get returns, without removing it from the set, a given
...
// Remove deletes the item, without returning it, identified by
...
// Iterator returns a channel of Hashable objects that is
